...
ggplot(total_agg1,  aes(x=Cases, y=Count, color=Cases)) +
    geom_boxplot() +
    #geom_point(aes(x=Cases, y=Count, col=(country=="Greece")))+
...
trel <- simpop %>%filter(date=="2020-05-17")%>%
 # group_by( country,confirmed, deaths, recovered, active) %>%
  summarise(country,confirmed, deaths, recovered, active)


library('lsa') 
p<-cosine(t(trel[-1]))

df = data.frame(Country = c("Haiti",  "Czech Republic", "Greece", "Dominican Republic","South Sudan") ,
                "Haiti" = as.numeric(p[ ,1 ]),
                "Czech Republic" = as.numeric(p[,2 ]),
                "Greece" = as.numeric(p[ ,3 ]),
                "Dominican Republic" = as.numeric(p[ ,4]),
                "South Sudan" = as.numeric(p[ ,5]) 
)


library(gridExtra)
grid.table(df)
png("similarity.png" , width = 1300 , height = 750 )
dev.off()
